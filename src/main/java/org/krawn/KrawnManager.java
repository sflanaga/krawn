...
public class KrawnManager {
    ...
    static void usage() {
        ...
        System.err.println(
                "nohup java -XX:+UseGCLogFileRotation -XX:NumberOfGCLogFiles=3 -XX:GCLogFileSize=16m -Xloggc:log/gc.log -verbose:gc -XX:+PrintGCDateStamps -Xmx128m -jar krawn.jar krawn.conf > it 2>&1 & ");
        System.exit(1);
    }
    ...
    public static void main(String[] args) {
        try {
            ...
            try {
                File lockfilename = new File(args[0] + ".lock"); 
                fis = new FileOutputStream(lockfilename, true);
                FileChannel chan = fis.getChannel();
                filelock = chan.tryLock();
                if (filelock == null) {
                    System.err.println("Unable to get exclusive ownership of the configuration sister file " + lockfilename + " - this is probably because ANOTHER INSTANCE OF THIS SAME PROGRAM IS RUNNING");
                    usage();
                }
